...
public class HexGridChunk : MonoBehaviour
{
	...
	void LateUpdate()// aggiorna la mesh
	{
		Triangulate();
		enabled = false;  //si disattiva finchè non serve
	}
	...
	//aggiunge il triangolo nella direzione indicata, versione con i multi triangoli
	...
	void TriangulateWithRiver(HexDirection direction, HexCell cell, Vector3 center, EdgeVertices e)
	{
		...
		center = Vector3.Lerp(centerL, centerR, 0.5f); //stabilisce l'ampiezza del fiume nella curva
		...
		if (!cell.IsUnderwater) {//no fiumi per le celle sommerse
	...
	//versione con multi triangoli, collega hex sui lati
	...
	// costruisce i terrazzamenti, versione multi triangolo
		...
		)
	{
		...
		Vector3 boundary = Vector3.Lerp(HexMetrics.Perturb(begin), HexMetrics.Perturb(right), b);  //senza perturb se tolgo i multi triangoli
		...
		if (leftCell.GetEdgeType( rightCell ) == HexEdgeType.Slope)
		{
			...
			terrain.AddTriangleUnperturbed(HexMetrics.Perturb(left), HexMetrics.Perturb(right), boundary);  //senza perturb se tolgo i multi triangoli
	...
	)
	{
		...
		Vector3 boundary = Vector3.Lerp(HexMetrics.Perturb(begin), HexMetrics.Perturb(left), b);  //senza perturb se tolgo i multi triangoli
	...
	{
		Vector3 v2 = HexMetrics.Perturb(HexMetrics.TerraceLerp(begin, left, 1));  //perturbato perche uso AddTriangleUnperturbed
		Color c2 = HexMetrics.TerraceLerp( beginColor, leftColor, 1 );

		terrain.AddTriangleUnperturbed(HexMetrics.Perturb(begin), v2, boundary);  //senza multi triangoli era AddTriangle
		...
		for (int i = 2; i < HexMetrics.terraceSteps; i++) {
			Vector3 v1 = v2;
			Color c1 = c2;
			v2 = HexMetrics.Perturb(HexMetrics.TerraceLerp(begin, left, i));  //perturbato perche uso AddTriangleUnperturbed
			c2 = HexMetrics.TerraceLerp( beginColor, leftColor, i );
			terrain.AddTriangleUnperturbed(v1, v2, boundary);  //senza multi triangoli era AddTriangle
			terrain.AddTriangleColor(c1, c2, boundaryColor);
			terrain.AddTriangleTerrainTypes(types);
		}

		terrain.AddTriangleUnperturbed(v2, HexMetrics.Perturb(left), boundary);  //senza multi triangoli era AddTriangle
		terrain.AddTriangleColor( c2, leftColor, boundaryColor );
		terrain.AddTriangleTerrainTypes(types);
	}

	//  per fare i multi triangoli, type indica che texture usare
	...
	{// considero celle d'acqua confinanti con l'acqua allo stesso livello di acqua
	...
	//sistema l'acqua verso la costa
	...
	//fiume e mare con elevazione diversa
	...
	//fiume e mare con elevazione uguale
